...
import SpProductDescription from "@/app/components/layout/sp-product-description/SpProductDescription";
...
const SingleProductPage = () => {
  ...
  const [showDeliveryEstimate, setShowDeliveryEstimate] =
  ...
  const [showQuantityWarning, setShowQuantityWarning] =
    useState<boolean>(false);
  const [showTileCalcWarning, setShowTileCalcWarning] =
    useState<boolean>(false);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const variationData = [
        { attribute: "Size", value: selectedVariation.name },
      ];
  ...
  return (
    <div className="bg-destructive-foreground h-auto">
      <div className="mx-auto py-6 md:py-12">
          ...
          <div className="w-full lg:pl-4">
            ...
            <div className="w-full flex flex-col gap-2">
              ...
              <div className="w-full flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                <strong className="text-xl md:text-2xl font-light sm:flex-3">
                  Select quantity
                </strong>
                {!isTilingProduct && (
                  <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                    <div className="relative">
                      ...
                      >
                        ...
                        <span>
                          <p className="text-sm">Tile Calculator</p>
                        </span>
                      </div>
                      <AnimatePresence>
                        {showTileCalcWarning && (
                    ...
                    <div className="w-max flex items-center gap-3 sm:gap-4 cursor-pointer relative transition-colors group">
                      <Switch
                        onCheckedChange={handleWastageToggle}
                        checked={addWastage}
                        className="cursor-pointer group-hover:bg-emperador/50! transition-colors duration-150"
                      />
                      <span>
                        <p className="text-sm">Add 10% wastage</p>
                      </span>
                      <CircleQuestionMark
                        onMouseLeave={() => setHoverPopup(false)}
                        onMouseOver={() => setHoverPopup(!hoverPopup)}
                        className="h-5 w-5 text-muted hover:text-emperador/70 duration-150"
                      />
                      {/* pop up  */}
                      {hoverPopup && (
                        <div className="absolute top-8 right-0 sm:right-0 bg-white w-64 sm:w-70 h-auto text-sm border border-dashed border-emperador/30 p-4 z-60">
                          ...
                          <p>
                            We recommend adding 10% for cuts and breakages, to
                            ensure you have enought for you project.
                          </p>
                        </div>
                      )}
                    </div>
            ...
            <Button
              ...
              className="w-full py-6 md:py-8 mt-6 md:mt-8 text-base md:text-lg uppercase"
            >
              Add to Cart
            </Button>
          </div>
        ...
        <SpProductDescription product={product} />
